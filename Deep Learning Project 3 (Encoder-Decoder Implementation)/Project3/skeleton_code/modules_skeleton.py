...
def train_model(trainloader, model, criterion, optimizer, scheduler, device):
    model.train()
    print(f'Total training iterations: {len(trainloader)}')
    for i, (inputs, labels) in enumerate(trainloader):
        from datetime import datetime
        print(f'Iteration in training: {i}')
...
def val_model(model, valloader, criterion, device, dir):
    ...
    print(f'Total validation iterations: {len(valloader)}')
    for batch, (inputs, labels) in enumerate(valloader):
        with torch.no_grad():
            print(f'Iteration in validation: {batch}')
