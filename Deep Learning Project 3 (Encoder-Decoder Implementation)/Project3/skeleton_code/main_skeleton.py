...
data_dir = "../dataset/VOCdevkit/VOC2012" #!changed it from ""
resize_size = 256
...
#! I take only 20 pictures in training and in val as well
trainset_20 = torch.utils.data.Subset(trainset, list(range(0, 20)))
valset_20 = torch.utils.data.Subset(valset, list(range(0, 20)))

print("trainLoader")
trainLoader = DataLoader(trainset_20, batch_size = batch_size, shuffle=True)
print("valLoader")
validLoader = DataLoader(valset_20, batch_size = batch_size, shuffle=True)
...
#!model.load_state_dict(torch.load("./trained_model/UNet_trained_model.pth", map_location=device))
model.load_state_dict(torch.load("./trained_model/resnet_encoder_unet.pth", map_location = device))
...
for epoch in range(epochs):
    ...
    if epoch % 4 == 0:
        ...
        torch.save(model.state_dict(), f'./{savepath2}')
