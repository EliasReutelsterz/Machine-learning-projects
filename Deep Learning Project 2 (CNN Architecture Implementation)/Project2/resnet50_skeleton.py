...
class ResNet50_layer4(nn.Module):
    def __init__(self, num_classes= 10): #blank# ): # Hint : How many classes in Cifar-10 dataset?
        super(ResNet50_layer4, self).__init__()
        self.layer1 = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=64, kernel_size=7, stride=2, padding=3),#blank#, #blank#, #blank#, #blank#, #blank# ),
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1)#blank#, #blank#, #blank#)
        )
        self.layer2 = nn.Sequential(
            ResidualBlock(in_channels=64, middle_channels=64, out_channels=256, downsample=False),#blank#, #blank#, #blank#, #blank#),
            ResidualBlock(in_channels=256, middle_channels=64, out_channels=256, downsample=False),#blank#, #blank#, #blank#, #blank#),
            ResidualBlock(in_channels=256, middle_channels=64, out_channels=256,downsample=True),#blank#, #blank#,#blank#, #blank#)
        )
        ...
        self.fc = nn.Linear(in_features=1024, out_features=num_classes)#blank#, #blank#) # Hint : Think about the reason why fc layer is needed
        self.avgpool = nn.AvgPool2d(kernel_size=2, stride=1)#blank#, #blank#)
